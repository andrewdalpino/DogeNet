...
class DogeDataset(Dataset):
    ...
    def __init__(self, root_path: str, train: bool = True, transformer: Optional[Transform] = None):
        ...
        bounding_boxes = [self._get_boxes(path.join(annotations_path, annotation)) for annotation in annotations]
    ...
    def __getitem__(self, index: int):
        filename, label, boxes = self.image_filenames[index], self.labels[index], self.bounding_boxes[index]

        image_path = path.join(self.images_path, filename)

        image = decode_image(image_path, mode=self.IMAGE_MODE)

        boxes = BoundingBoxes(boxes, format=self.BOX_FORMAT, canvas_size=image.shape[-2:])
